...
private struct CaptureArguments {
    ...
    init?() {
        ...
        guard state == "default" || state == "menu-open" else {
            Self.printUsage()
            return nil
        }
    ...
    private static func printUsage() {
        let usage = "Usage: capture.swift <route> <width> <height> <output.png> [default|menu-open]\n"
...
private func preparedDocument(siteRoot: URL) throws -> String {
    ...
    var appJavaScript = try source(at: siteRoot.appendingPathComponent("app.js"))
...
(function () {
  ...
  return JSON.stringify({
    ...
    activeNav: Array.prototype.map.call(document.querySelectorAll('nav a.active'), function (anchor) {
...
private final class CaptureDelegate: NSObject, WebFrameLoadDelegate {
    ...
    func webView(_ webView: WebView!, didFinishLoadFor frame: WebFrame!) {
        ...
        if arguments.state == "menu-open" {
            _ = webView.stringByEvaluatingJavaScript(
                from: "var panel=document.querySelector('.site-nav');if(panel){panel.style.transition='none';}var button=document.querySelector('.menu-toggle');if(button){button.click();}"
            )
        }

        let delay = arguments.state == "menu-open" ? 0.35 : 0
